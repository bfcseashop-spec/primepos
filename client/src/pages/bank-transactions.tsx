...
export default function BankTransactionsPage() {
  ...
  const { data: transactionsData, isLoading } = useQuery<{ items: BankTransaction[]; total: number; totalDeposits?: number; totalWithdrawals?: number }>({
    ...
    queryFn: async () => {
      ...
      params.set("limit", String(Math.max(1, txPageSize)));
  ...
  const totalBillCollections = dateFilteredBills.reduce((s: number, b: any) => s + (Number(b.paidAmount) || 0), 0);
  ...
  const dueTotal = dateFilteredBills.filter((b: any) => b.status !== "paid").reduce((s: number, b: any) => s + (Number(b.total) - Number(b.paidAmount)), 0);

  const filteredBillPayments = dateFilteredBills.filter((b: any) => {
