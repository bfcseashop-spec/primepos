            ...
            return (
              <SidebarMenuItem key={item.title}>
                <SidebarMenuButton
                  ...
                  isActive={active}
                  className={active
                    ? "bg-gradient-to-r from-blue-500/10 to-violet-500/10 dark:from-blue-600/20 dark:to-violet-600/20 border border-blue-500/15 dark:border-white/[0.08] rounded-md"
                    : "border border-slate-200 dark:border-white/[0.04] rounded-md bg-white/60 dark:bg-white/[0.02]"}
                >
                  <Link href={item.url} data-testid={`link-nav-${item.title.toLowerCase().replace(/\s+/g, '-')}`}>
                    <span className={`inline-flex items-center justify-center rounded-md p-1 ${active ? `bg-gradient-to-br ${item.gradient} shadow-sm` : "bg-slate-100 dark:bg-white/5"}`}>
                      <item.icon className={`h-3.5 w-3.5 ${active ? "text-white" : "text-slate-400 dark:text-slate-400"}`} />
                    </span>
                    <span className={`text-[13px] font-semibold ${active ? "text-slate-900 dark:text-white" : "text-slate-600 dark:text-slate-300"}`}>{item.title}</span>
                    {active && (
                      <ChevronRight className="ml-auto h-3.5 w-3.5 text-blue-500/60 dark:text-blue-400/60" />
                    )}
                  </Link>
                </SidebarMenuButton>
              </SidebarMenuItem>
